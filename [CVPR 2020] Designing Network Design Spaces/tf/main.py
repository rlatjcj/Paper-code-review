...
def select_config(args, cfg):
    for k, v in cfg[args.backbone.upper()].items():
        if k == 'n_block':
            setattr(args, k, [random.choice(range(1, v+1)) for _ in range(args.n_stage)])
        elif k == 'n_channel':
            channel_list = []
            i = 1
            val = 8
            while val*i <= v:
                channel_list.append(val*i)
                i += 1

            setattr(args, k, [random.choice(channel_list) for _ in range(args.n_stage)])
        elif k in ['bottleneck_ratio', 'group_width']:
            setattr(args, k, np.random.choice(v))
        else:
            setattr(args, k, v)
...
if __name__ == "__main__":
    ...
    # model
    parser.add_argument("--backbone",       type=str,       default='anynet',       choices=['anynet', 'regnet'])
